...
img_clip <- function(img_dir,proc_dir,gpkg){
  imgs <- list.files(path=img_dir,pattern="(.tif|.tiff|.TIF|.TIFF)$",all.files=FALSE,full.names=FALSE)
  ...
  for (i in imgs){
    ...
    joined <- st_join(gpkg,st_as_sf(data_bound,crs=crs(data)),sparse=F)
    st_write(joined,file.path(proc_dir,paste(tools::file_path_sans_ext(i),".gpkg",sep="")),append=FALSE)
    img_crop <- crop(data,st_bbox(joined))
    writeRaster(img_crop,file.path(proc_dir,paste(base_name,"_clips",sep=""),i),overwrite=TRUE)
  }
